...
async def admin_dashboard(request: Request):
    ...
    <style>
    .admin-container {{
        max-width: 700px; margin: 3em auto; background: #fff; border-radius: 14px;
        box-shadow: 0 2px 16px #0001; padding: 2.5em; font-family: 'Segoe UI',Arial,sans-serif;
    }}
...
async def upload_orders_display(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        for _, row in unmatched.iterrows():
            unmatched_report.append({'order_no': row['Order number'], 'sku': row['Offer SKU'], 'qty': int(row['Quantity'])})
    except Exception as e:
